# -*- coding: utf-8 -*-
"""Plot core training curves for a recurrent QMIX run."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot RNN QMIX simple_spread run.")
...
def read_random_baseline(run_dir: Path) -> float | None:
    ...
    if not path.is_file():
        print(f"warning: matching random baseline not found; expected {path}")
        return None
...
def save_learning_status(
    ...
    points = [
        (to_float(row.get("episode")), to_float(row.get("eval_team_return")))
        for row in rows
    ]
    ...
    if points:
        ...
        plt.plot(episodes, values, marker="o", label="RNN QMIX eval return")
        best_episode, best_value = max(points, key=lambda item: item[1])
        plt.scatter(
            [best_episode],
            [best_value],
            s=70,
        ...
        plt.annotate(
            f"best episode={int(best_episode)}, return={best_value:.2f}",
        ...
        print(f"Best eval point: episode={int(best_episode)}, eval_team_return={best_value}")
    ...
    plt.title(f"RNN QMIX simple_spread learning status ({mixer_type})")
...
def save_training_health(rows: list[dict], run_dir: Path, mixer_type: str = "qmix") -> Path:
    ...
    series = {
        "td_loss": [to_float(row.get("td_loss")) for row in rows],
        ...
        "action_entropy_mean": [to_float(row.get("action_entropy_mean")) for row in rows],
        "used_action_count_min": [to_float(row.get("used_action_count_min")) for row in rows],
        "max_action_ratio_max": [to_float(row.get("max_action_ratio_max")) for row in rows],
        "top2_action_ratio_max": [to_float(row.get("top2_action_ratio_max")) for row in rows],
    }
    ...
    fig.suptitle(f"RNN QMIX training health ({mixer_type})")
    fig.tight_layout()
