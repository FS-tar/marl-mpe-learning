# -*- coding: utf-8 -*-
"""QMIX and VDN mixers for recurrent episode batches."""
...
class QMixer(nn.Module):
    """Monotonic QMIX mixer supporting [batch, n_agents] or [batch, T, n_agents]."""
    ...
    def forward(self, agent_qs: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
        ...
        if agent_qs.dim() == 3:
            batch_size, seq_len, n_agents = agent_qs.shape
            if n_agents != self.n_agents:
                raise RuntimeError(f"expected {self.n_agents} agents, got {n_agents}")
            agent_qs = agent_qs.reshape(batch_size * seq_len, n_agents)
            states = states.reshape(batch_size * seq_len, self.state_dim)
            restore_shape = (batch_size, seq_len, 1)
        elif agent_qs.dim() == 2:
            batch_size, n_agents = agent_qs.shape
            if n_agents != self.n_agents:
                raise RuntimeError(f"expected {self.n_agents} agents, got {n_agents}")
            restore_shape = (batch_size, 1)
        else:
            raise RuntimeError(
                f"QMixer expects agent_qs [batch, n_agents] or [batch, T, n_agents], "
                f"got {tuple(original_shape)}"
            )

        flat_batch = agent_qs.shape[0]
        if states.shape != (flat_batch, self.state_dim):
            raise RuntimeError(
                f"states must be [{flat_batch}, {self.state_dim}], got {tuple(states.shape)}"
            )

        agent_qs = agent_qs.view(flat_batch, 1, self.n_agents)
        ...
        b1 = self._bounded_bias(self.hyper_b1(states)).view(flat_batch, 1, self.hidden_dim)
        ...
        v = self._bounded_bias(self.v(states)).view(flat_batch, 1, 1)
        q_tot = torch.bmm(hidden, w_final) + v
        return q_tot.view(*restore_shape)
...
class VDNMixer(nn.Module):
    ...
    def forward(self, agent_qs: torch.Tensor, states: torch.Tensor | None = None) -> torch.Tensor:
        ...
        raise RuntimeError(
            f"VDNMixer expects agent_qs [batch, n_agents] or [batch, T, n_agents], "
            f"got {tuple(agent_qs.shape)}"
        )
